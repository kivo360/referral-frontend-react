...
export default class Analysis extends Component {
      ...
      return (
        <List.Item key={item.email}>
          <List.Item.Meta
            ...
            title={
              <span>
                <a className={styles.username} style={{ marginRight: 10 }}>
                  {item.email}
                </a>
                &nbsp;
              </span>
            ...
            description={
              <span className={styles.datetime} title="Whats going on here?">
                <div>
                  ...
                  <span>
                    <h4>{item.degree} Degree Referral</h4>
                  </span>
                </div>
              </span>
            }
  ...
  renderPopover(number, degree) {
    return (
      <div>
        <p>
          You need to have at least <strong>{number}</strong> referrals for this in the{' '}
          <strong> {degree} </strong> level
        </p>
      </div>
    );
  }
    ...
    return (
      <Fragment>
        ...
        <div style={{ textAlign: 'right', paddingTop: '1.2rem' }}>
          <CopyToClipboard
            text={this.state.refHash}
            onCopy={() => message.success('Copied referral link! Make them invite referrals!')}
          >
            <span>
              <a>{this.state.refHash}</a>
            </span>
          </CopyToClipboard>
        </div>
        <Card
                  ...
                  <PieChart>
                    ...
                    >
                      {data01.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={dataColors[Math.floor(Math.random() * dataColors.length)]}
                        />
                      ))}
                    </Pie>
                    <Tooltip />
                    ...
                    {item.over ? (
                      <Card.Grid style={benefitStyleSuccess}>
                        ...
                        <h3 style={{ textAlign: 'center' }}>
                          Progress So Far: {((degrees[item.degree] / item.number) * 100).toFixed(2)}%
                        </h3>
                      </Card.Grid>
                    ) : (
                      <Popover
                        content={this.renderPopover(item.number, item.degree)}
                        title="Requirements"
                      >
                        <Card.Grid className={styles.prizeGrid}>
                          ...
                          <h3 style={{ textAlign: 'center' }}>
                            Progress So Far:{' '}
                            {((degrees[item.degree] / item.number) * 100).toFixed(2)}%
                          </h3>
                        </Card.Grid>
                      </Popover>
                    )}
